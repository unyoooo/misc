...
| **充電しながら使える電源シミュレータ** (`powerPathSim.html`) | MP2636(パワーパス充電＋昇圧・最大3A)で「USB接続中は入力から給電しながら充電、抜いたら電池から昇圧」する電源を設計する。RISET/RILIM/ROLIM/R1R2をデータシートの式から計算(E24・E96近似)、入力電流の取り合い(負荷優先で充電が絞られる挙動)をグラフ化。充電カーブ・ランタイム・発熱、ステータスLEDとNTC温度窓の抵抗計算、電源スイッチの入れ方、保護回路(過放電は非対応)の解説つき | [起動](https://htmlpreview.github.io/?https://github.com/unyoooo/misc/blob/master/powerPathSim.html) |
